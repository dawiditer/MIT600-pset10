...
class Hand(object):
    ...
    def update(self, word):
        ...
        for letter in word_freq_dict:
            self.handDict[letter] = self.handDict[letter] - word_freq_dict[letter]
            if self.handDict[letter] == 0: del self.handDict[letter]
    ...
    def containsLetters(self, letters):
        ...
        """
        # TODO
        #check all letters in letters if in hand
        letters_freq_dict = getFrequencyDict(letters)
        for letter in letters_freq_dict:
            if letter not in self.handDict or self.handDict[letter] - letters_freq_dict[letter] < 0:
                return False
        return True
    def isEmpty(self):
        """
    ...
    def __eq__(self, other):
        ...
        """
        # TODO
        #if they has the exact same letters and frequency
        #ie same keys and values
        return self.handDict.keys() == other.handDict.keys() and self.handDict.values() == other.handDict.values()
    def __str__(self):
        """
...
class Player(object):
    ...
    def addPoints(self, points):
        ...
        """
        # TODO
        #modifier, not returning anything
        self.points += points
...
class ComputerPlayer(Player):
    ...
    def pickBestWord(self, wordlist):
        ...
        # TODO
        #self.hand        
        #if the letters in hand are all in word
        #self.hand.containsLetters(word)
        
        best_score = 0
        best_word = "."
        for word in wordlist.getList():#loop through the wordlist
            word_score = getWordScore(word)
            #if the current word contains the letters in hand
            if self.hand.containsLetters(word) and word_score > best_score:
                best_score,best_word = word_score,word
        return best_word
